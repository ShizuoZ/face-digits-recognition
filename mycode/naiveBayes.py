...
class NaiveBayesClassifier(classificationMethod.ClassificationMethod):
  ...
  def train(self, trainingData, trainingLabels, validationData, validationLabels):
    ...
    if (self.automaticTuning):
        kgrid = [0.001, 0.01, 0.1, 0.5, 1, 1.3, 2, 5, 10, 20, 50]
    else:
        kgrid = [self.k]
    # self.loadWeight()
  ...
  def saveWeight(self, filename):
    f = open(filename,'wb')
    cPickle.dump(self.prior,f,-1)
    cPickle.dump(self.conditionalProb,f,-1)
    f.close()
  ...
  def loadWeight(self, filename):
    ...
    f = open(filename,'rb')
    self.prior = cPickle.load(f)
    self.conditionalProb = cPickle.load(f)
    f.close()
